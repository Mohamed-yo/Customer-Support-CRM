...
export default function DepartmentsPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormError(code === 'name_in_use' ? t('departments.nameInUse') : t('departments.saveFailed'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleToggleActive = async (department: Department) => {
    try {
      ...
      setError(t('departments.actionFailed'));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-2xl sm:text-3xl font-semibold text-slate-800">{t('departments.title')}</h1>
        <button
          type="button"
        ...
        <div className="overflow-x-auto rounded border border-slate-200 bg-white">
          {departments.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-1 px-4 py-16 text-center">
              <p className="text-sm text-slate-500">{t('departments.empty')}</p>
            </div>
          ) : (
              ...
              <tbody>
                {departments.map((d) => (
                  <tr key={d.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600">{d.isActive ? t('departments.active') : t('departments.inactive')}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className={`rounded px-2.5 py-1.5 text-sm font-medium hover:bg-slate-100 ${d.isActive ? 'text-red-600 hover:bg-red-50' : 'text-slate-700'}`}
                        >
                          {d.isActive ? t('departments.deactivate') : t('departments.reactivate')}
                        </button>
                      </div>
                    </td>
                  </tr>
      ...
      {formOpen && (
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold text-slate-800">{editingId ? t('customers.edit') : t('departments.new')}</h2>

            <label className="flex flex-col gap-1 text-sm text-slate-700">
              <span>{t('departments.form.name')}</span>
              <input
                type="text"
                value={name}
